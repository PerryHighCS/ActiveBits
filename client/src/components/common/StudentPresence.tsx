...
export function StudentPresencePanel({ isOpen, onClose, entries, title = 'Connected Students', controlsId, showDisconnected = false, renderRowActions, renderRowContent, renderBadges, getRowClassName, getRowStyle }: { isOpen: boolean; onClose: () => void; entries: StudentPresenceEntry[]; title?: string; controlsId: string; showDisconnected?: boolean; renderRowActions?: (entry: StudentPresenceEntry) => React.ReactNode; renderRowContent?: (entry: StudentPresenceEntry) => React.ReactNode; renderBadges?: (entry: StudentPresenceEntry) => React.ReactNode; getRowClassName?: (entry: StudentPresenceEntry) => string | undefined; getRowStyle?: (entry: StudentPresenceEntry) => React.CSSProperties | undefined }) {
  ...
  const visible = useMemo(() => {
    ...
    return entries.filter((entry) => (showDisconnected || entry.connected) && `${entry.displayName} ${entry.participantId}`.toLowerCase().includes(normalizedQuery))
  }, [entries, query, showDisconnected])
  useEffect(() => {
    const focusedEntryId = focusedEntryIdRef.current
    if (!isOpen || !focusedEntryId || visible.some((entry) => entry.participantId === focusedEntryId)) return
  ...
  const close = () => { onClose(); if (typeof document !== 'undefined') document.getElementById(`${controlsId}-toggle`)?.focus() }
  return <aside id={controlsId} className="h-full w-80 border-l border-gray-200 bg-white shadow-lg overflow-hidden" aria-hidden="false" aria-labelledby={titleId}>
    <div className="h-full flex flex-col"><div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between"><h2 id={titleId} className="text-base font-semibold text-gray-800">{title}</h2><button type="button" onClick={close} onFocus={() => { focusedEntryIdRef.current = null }} aria-label={`Close ${title}`} className="text-sm text-gray-600 hover:text-gray-900">Close</button></div>
      <div className="p-4"><label className="sr-only" htmlFor={`${controlsId}-search`}>Search students</label><input ref={searchInputRef} id={`${controlsId}-search`} value={query} onFocus={() => { focusedEntryIdRef.current = null }} onChange={(event) => setQuery(event.target.value)} placeholder="Search students" className="w-full rounded border border-gray-300 px-2 py-1 text-sm" /></div>
      <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-2">{visible.length === 0 ? <p className="text-sm text-gray-600">{getStudentPresenceEmptyMessage(query, showDisconnected)}</p> : visible.map((entry) => <div key={entry.participantId} onFocus={() => { focusedEntryIdRef.current = entry.participantId }} className={`flex items-center justify-between gap-2 px-3 py-2 rounded border border-gray-200 bg-gray-50 ${getRowClassName?.(entry) ?? ''}`} style={getRowStyle?.(entry)}><div className="min-w-0">{renderRowContent?.(entry) ?? <p className="text-sm font-medium text-gray-800 truncate">{entry.displayName}</p>}{renderBadges?.(entry)}</div>{renderRowActions?.(entry)}</div>)}</div>
    </div>
  </aside>
}
