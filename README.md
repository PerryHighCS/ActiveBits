...
Student-specific activity state is authorized with server-issued, session-scoped httpOnly participant cookies; student IDs retained in browser state are routing hints only and are never trusted for authorization.
