...
- **Linked-session keepalive**: A session may refresh one directly linked session on genuine activity; the relationship is deliberately single-hop to keep cycles safe. An active Learn entry relies on this store-backed lifetime rather than a separate logical-expiry gate.
