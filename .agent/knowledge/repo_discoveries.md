...
- Status: **SUPERSEDED 2026-09-02 (PR #365, commit 58eed143).** The `navigator.userActivation` gesture-recency fix below was replaced, not shipped.
- Discovery: With more than one Video Sync manager view connected, the `onStateChange` -> `sendCommand` mirror created a feedback loop. The manager player is unmuted, so a programmatic `playVideo()` on a second, non-interacted manager is refused by the browser autoplay policy; that manager's player then emits involuntary `PAUSED` (autoplay fallback, per-heartbeat `seekTo` once drift exceeds the 2s tolerance, buffering/stall events) which the mirror sent back as `pause` commands, re-pausing playback for everyone every few seconds.
- Why it matters: The root problem was that the manager mirrored *any* `onStateChange` transition as a command. **Shipped fix:** stop mirroring entirely. The manager iframe now renders `controls: 0` / `pointer-events-none` / `aria-hidden` (a projection), playback authority is a set of activity-owned Play/Pause/Seek controls that issue `POST /command` with a `commandId` + `managerId`, and the only `onStateChange` path left is a natural `ENDED` -> `pause`. A non-interacting second manager therefore cannot mirror ordinary playback transitions as commands; only a natural `ENDED` can still emit a `pause`. The server accepts that `pause` from any capability-holding manager whose command names the current `playbackRevision` and reports a plausible end position (>= `startSec`, and within `NATURAL_END_TOLERANCE_SEC` of `stopSec` when set) - **not** bound to `controllerId`, because that per-page id stranded playback as `isPlaying: true` once the Play-issuing manager reloaded/closed/was autoplay-blocked and only another manager's player reached the media end (with no `stopSec` the server has no other end detector). The gesture-recency heuristic (`navigator.userActivation`, `MANAGER_USER_GESTURE_GRACE_MS`, `isManagerPlaybackGestureRecent`, `readManagerUserActivation`) was removed — those APIs no longer exist. The autoplay-blocked "Click to start playback" affordance did ship. The surviving `ENDED` -> `pause` path (`shouldEmitNaturalEndPause`) is guarded three ways against a delayed `ENDED` from a superseded playback: (1) the mirrored `expectedPlaybackRevision` is the revision this player was actually driving (`playerAppliedRevisionRef`, captured in `applyStateToPlayer`, not `latestStateRef` at callback time); (2) `isNaturalPlaybackCompletion` requires the playhead within 2s of `getDuration()`; (3) a monotonic *playback generation* — `applyStateToPlayer` bumps `playerPlaybackGenerationRef` on every fresh `playVideo()`, `onStateChange` records it on each PLAYING transition, and an `ENDED` whose generation no longer matches the last PLAYING generation is rejected. (3) is what catches the residual the proximity+revision checks miss: a replay whose `startSec` sits inside the 2s end window, where the stale `ENDED` lands after the new `playVideo()` but before the new PLAYING event. Anything still slipping through is heartbeat-reconciled. Delivery of that `pause` is a **bounded auth-retry** (`emitNaturalEndPause`), not a fire-and-forget `sendCommand`: a transient 401/403 while the capability cookie refreshes would otherwise drop it, and with no `stopSec` the server would keep the finished video `isPlaying: true`. It reuses the instructor queue's `resolveManagerPlaybackFlushOutcome` retry/drop decision, and `shouldSendNaturalEndPause` aborts the retry once `playerEndedRef` is cleared or `playbackRevision` advances past the ended revision.
- Evidence: `activities/video-sync/client/manager/VideoSyncManager.tsx` (activity-owned controls, `autoplayBlocked`); `.agent/knowledge/data-contracts.md` (2026-09-02 "Video Sync REST | websocket | multi-instructor playback").
- Follow-up action: If Video Sync ever needs an explicit "driver" among multiple managers, prefer a server-assigned control token. `controllerId` on the playback state records the last commander for telemetry/labelling only - natural-`ENDED` acceptance is no longer bound to it (see above).
- **Known limitation of the current design:** replay-after-natural-end is only recoverable by the *emitting* manager. `resolveExplicitPlaybackPositionSec` restarts Play from `startSec` when (a) this page's `playerEndedRef` is set (natural `ENDED` seen here) or (b) authoritative `positionSec >= stopSec` (any manager, when `stopSec` is configured — commit `04b05ee7`). With **no `stopSec`**, a manager that did not see the `ENDED` (a second manager, or the same one after reload) has `playerEnded === false` and no boundary, so Play sends no `positionSec`, the server resumes from the persisted end position, and the next heartbeat re-seeks everyone to the end (issue #245 shape). Closing this needs an end-of-media / replay marker in authoritative `state` (cleared on any seek / play-from-position / reconfigure) so every manager derives "ended" the same way — a protocol addition (new `state` field → server routes, normalization, sync math, `ACTIVITY_PAYLOADS.md`). Tracked in **#368** (raised in the PR #365 review).
- Owner: Claude
...
- Why it matters: Future runner implementations can add ids to the shared `MobCodeRunnerId` union and runner registry without changing shared dashboard code. If classrooms need offline or locked-down execution, the Brython assets are already self-hosted (no CDN dependency).
- Evidence: `activities/mobcode/client/runner/runnerUtils.ts`; `activities/mobcode/client/manager/MobCodeManager.tsx`; `activities/mobcode/client/student/MobCodeStudent.tsx`; `activities/mobcode/server/routes.ts`; `skills/syncdeck/references/ACTIVITY_PAYLOADS.md`; `DEPLOYMENT.md`
- Follow-up action: Add an xterm-backed terminal renderer, nested-function input coverage, package/relative import support, writable session-local files if needed, and the CMU graphics bridge as incremental Brython runner improvements rather than replacing the MobCode editor transport.
- Owner: Codex
...
- Date: 2026-05-06
- Area: activities
- Discovery: `video-sync` supports YouTube and YouTube Education URLs by parsing `youtube.com/watch?v=<id>`, `youtube.com/embed/<id>`, `youtubeeducation.com/watch?v=<id>`, and `youtubeeducation.com/embed/<id>` to the same 11-character YouTube video ID. Education URLs persist `state.playerHost = "youtube-education"` so manager and student clients try the education player first, then transparently fall back to `youtube-nocookie` if the education player errors or does not become ready.
- Why it matters: Google Slides may expose or internally use YouTube Education-shaped URLs while falling back to ordinary YouTube playback behind the scenes. Video Sync should accept those URL formats, plus ordinary YouTube embed codes, while still recovering to the proven no-cookie player when `youtubeeducation.com` rejects an otherwise-valid video.
...
- Date: 2026-09-04
- Area: activities (video-sync) | legacy session identity
- Discovery: `getSessionCreatedIdentity(session)` returns `null` for a legacy record (no persisted `created`) even though `toSessionRecord` gives it a synthetic `created: Date.now()` that differs on every strict read. `activities/video-sync/server/routes.ts` derives its unsynced-student bookkeeping scope from that identity: legacy -> `<id>:0`, modern -> `<id>:<created>`. Commit a53ae43f fixed the heartbeat and `/event` derivations to `getSessionCreatedIdentity(session) ?? undefined` but left five `updateConnectionTelemetry(..., session.created)` / `currentSession.created` call sites (session GET, config PATCH, command POST, socket cleanup, initial snapshot) passing the raw synthetic number - so for a legacy session those paths refreshed/persisted `telemetry.sync.unsyncedStudents` against a per-read-random key (effectively always 0), clobbering the count `/event` maintained under `<id>:0`. Fixed by making all five use the same identity helper, plus `migrateLegacyUnsyncedScope()` which folds any `<id>:0` markers into `<id>:<created>` (2-key `video-sync-unsynced-merge` Lua for Valkey, in-memory map move otherwise) once per session per process the first time a numeric identity is seen - covering the one-time transition when the first atomic write persists the synthesized `created`.
- Why it matters: Legacy video-sync sessions (pre-`created`-migration records still live in Valkey) otherwise show a stuck/zeroed unsynced-students telemetry count for instructors, and lose any markers recorded before their first atomic write.
- Evidence: `activities/video-sync/server/routes.ts` (`migrateLegacyUnsyncedScope`, `MERGE_UNSYNCED_STUDENTS_LUA`, `migratedLegacyUnsyncedScopes`); `activities/video-sync/server/routes.test.ts` ("legacy session unsynced markers survive the scope move once created is persisted"); `server/core/sessions.ts` (`getSessionCreatedIdentity`, `legacyCreatedSessions`, `toSessionRecord`).
- Follow-up action: `migratedLegacyUnsyncedScopes` is a process-level guard cleared on session teardown; a legacy id reused by another legacy incarnation within one process could skip its fold (self-heals via the 21s key TTL). Revisit if legacy records are ever backfilled with a deterministic `created`.
- Owner: Claude

- Date: 2026-09-04
- Area: activities (video-sync) | websocket admission
- Discovery: The `/ws/video-sync` handler wires up `ensureBroadcastSubscription` + `upsertSubscriber` + `ensureHeartbeat` before the `withSessionMutation` block that reads/persists the initial snapshot. That block's `.catch` previously only logged `initial-snapshot-failed`, so a socket whose snapshot mutation threw (Valkey outage, exhausted CAS retries) stayed in `subscribersBySession` and kept receiving later broadcast envelopes despite never getting an authoritative snapshot, and its heartbeat kept running. Fixed: the `.catch` now calls the idempotent `handleSocketClosed()` (removes subscriber, rebroadcasts connection count, stops the orphaned heartbeat) and closes the socket `1011`, but only when `isSubscribed` is true so the not-found / forbidden paths (which close the socket themselves before subscribing) are untouched.
- Evidence: `activities/video-sync/server/routes.ts` (ws `register('/ws/video-sync')` trailing `.catch`); `activities/video-sync/server/routes.test.ts` ("websocket initial snapshot failure unsubscribes the socket instead of leaving it in the broadcast set", "invalid websocket session still closes when sending the not-found envelope throws").
- Owner: Claude

- Date: 2026-09-04
- Area: activities (video-sync) | legacy session identity (follow-up)
- Discovery: The first `migrateLegacyUnsyncedScope()` fix (see the entry above from earlier the same day) triggered ambiently from `updateConnectionTelemetry`/`/event` any time a session's *current* read showed a numeric `created`, with no proof that the `<id>:0` markers it folded in belonged to that same incarnation - a session id deleted and reused (the new incarnation never legacy, `created` set from creation) could inherit a still-live, unrelated `<id>:0` residue within its 21s TTL window. Fixed by moving the trigger inside `updateVideoSyncSessionAtomic` itself, gated on `expectedCreated === null` (every call site already passes `{ expectedCreated: getSessionCreatedIdentity(session) }` from its own pre-write read, so `=== null` is proof, not inference, that the read immediately before *this* write saw the record as legacy) plus a non-null `getSessionCreatedIdentity` on the just-committed result. A read-only path (heartbeat/GET with nothing to persist, or a fresh non-legacy session) never sets `expectedCreated: null` from a legacy read, so it never triggers the fold.
- Why it matters: Ties the fold to the exact write that mints a legacy record's first `created`, closing the cross-incarnation leak while still covering the real (single-incarnation) transition.
- Evidence: `activities/video-sync/server/routes.ts` (`updateVideoSyncSessionAtomic`, `migrateLegacyUnsyncedScope`); `activities/video-sync/server/routes.test.ts` ("legacy session unsynced markers survive the scope move triggered by that session's own first persisting write", "a fresh session reusing a legacy id does not inherit that id's stale :0 unsynced markers").
- Owner: Claude

- Date: 2026-09-04
- Area: activities (video-sync) | manager natural-end retry
- Discovery: The natural-end pause retry (see the commandId-reuse entry from earlier the same day) had no guard against overlapping episodes: nothing prevented the player reaching ENDED a second time (a fresh play/seek followed by another natural completion) while an older episode's `sendCommand` await was still in flight, so an older episode's completion handler could run *after* a newer episode had already reset the shared `naturalEndRetryCountRef` / `naturalEndCommandIdRef` for itself, clobbering the newer episode's state (or scheduling a stale retry with the old episode's revision/position). Fixed by minting a monotonic `episodeToken` at the one place a fresh episode begins (the player's ENDED handler) and threading it through `emitNaturalEndPause`'s `params` (so retries of the *same* episode automatically carry the same token); `naturalEndEpisodeOwnerRef` holds the current owner, checked via the new pure `resolveNaturalEndPauseCompletion` helper after every `await sendCommand`, mirroring the existing `playbackFlushOwnerRef` pattern used for the instructor-gesture queue. Every site that resets `naturalEndRetryCountRef`/`naturalEndCommandIdRef` (session change, player teardown) also resets `naturalEndEpisodeOwnerRef.current = 0` so a stale in-flight attempt from the torn-down context can't act after the fact.
