...
- Follow-up action: If Video Sync ever needs an explicit "driver" among multiple managers, prefer a server-assigned control token; `controllerId` on the playback state is the current lightweight version (last commander owns natural-`ENDED`).
- Owner: Claude
