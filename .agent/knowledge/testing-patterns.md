...
- Pattern: For MobCode editor sync, test both sides of stale durable snapshot handling: editable manager clients should ignore remote durable `state-sync`/`file-tree-changed` echoes, and the server should prefer the latest live websocket group over an older ordinary `state-sync` POST payload only while an active manager socket is connected.
...
- Follow-up action: If MobCode later adds explicit revision numbers, update these tests to assert monotonic revision handling instead of relying on manager-gated live-state preference.
- Owner: Codex
