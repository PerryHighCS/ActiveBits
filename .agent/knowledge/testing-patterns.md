...
- Date: 2026-09-02
- Pattern: A route that calls `sessions.updateAtomic` needs a test store that actually implements a revision-checked CAS, or the test silently exercises the non-atomic fallback. `activities/video-sync/server/routes.test.ts` `createSessionStore` takes an opt-in `atomic: true` (adds `getStrict` + a real `updateAtomic` retry loop over the shared store) plus an `onAtomicAttempt(attempt, store)` hook that mutates the store mid-loop to force a compare-and-set conflict. Keep it opt-in: flipping it on by default reroutes every existing route test and breaks `strictGetCalls` / `set`-count assertions.
- Evidence: `activities/video-sync/server/routes.test.ts` ("command route retries its atomic write and does not clobber a revision advanced by another instance", "command route atomic path accumulates the playback revision across sequential commands").
