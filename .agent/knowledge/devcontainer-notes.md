...
- Date: 2026-03-20
- Change: Keep `postCreateCommand` lightweight and workspace-scoped. ActiveBits now uses `.devcontainer/post-create.sh` only for `safe.directory` registration plus workspace dependency install, while `.devcontainer/setup-dev.sh` remains a `postStartCommand` concern.
