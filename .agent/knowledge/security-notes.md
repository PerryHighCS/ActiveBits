...
- Control or mitigation: Added `toPublicSession()` in `server/core/sessions.ts`, which returns the session unchanged unless `session.data` has `participantAuthTokens`, in which case it returns a shallow copy with that key omitted (the underlying stored record and every other field, including `acceptedEntryParticipants`, are untouched). `GET /api/session/:sessionId` now returns `toPublicSession(session)`. It is the only route in this file that returns a raw session object.
- Residual risk: `session.data.entryParticipants` (pending, not-yet-accepted waiting-room submissions, keyed by a one-time consume token) is not redacted by this fix — that token is short-lived and single-use rather than a durable bearer credential, and was not the concern in this finding. Also **not fixed here**: `session.data.instructorPasscode`, stored directly by several activities (confirmed for Resonance and video-sync), is returned by this same unredacted route today — a pre-existing exposure that predates this PR and was not flagged by either reviewer. Worth a dedicated follow-up.
- Validation (test/review/path): `server/sessionEntryRoutes.test.ts` ("unauthenticated session read redacts participant bearer tokens").
- Follow-up action: If another durable secret is ever added to any activity's `session.data`, redact it here too (or replace the denylist with an explicit per-activity allowlist) rather than assuming this route is safe by default. Separately: audit and fix the pre-existing `instructorPasscode`-in-`session.data` exposure through this same route.
