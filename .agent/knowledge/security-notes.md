...
- Control or mitigation: Public stores now always mint IDs. A separate trusted store preserves an ID only for a SyncDeck child handoff authorized by the parent accepted-entry cookie; embedded handoffs also require the current roster, while solo handoffs do not because standalone students have no roster record. Residual risk: the solo route accepts any existing non-embedded child ID, so an accepted student can enter an unrelated known session under their own server-issued ID; parent binding is tracked in #388. SyncDeck normalizes and retains the token map used to verify that cookie. Its embedded WS and HTTP token paths do not issue child tokens to unverified student IDs; the solo overlay uses a parent-cookie-authorized endpoint.
