...
- Compatibility constraints: SyncDeck owns the released-slide detection and may request the flag multiple times, but the flag is best-effort and should only matter while Resonance has no active question run yet. Resonance owns converting that flag into an initial "all questions active" run, and must clear/consume the flag so later manual deactivation does not immediately re-enable all questions on the next normalization pass.
- Validation rules: SyncDeck's student-triggered auto-activate request must be student-authenticated against the parent session and only target an existing embedded child session for that `instanceKey`. Resonance should activate questions in their stored order, reuse normal countdown aggregation logic, and leave non-Resonance embedded activities untouched.
...
- Owner: Codex
- Contract: SyncDeck static presentation hosting should use the same-origin utility route `/util/syncdeck/launch-presentation?presentationUrl=<absolute-url>` rather than browser-side cross-origin calls to `/api/syncdeck/*`. The presentation supplies only `presentationUrl`; ActiveBits runs SyncDeck preflight, creates/configures a standalone SyncDeck session on its own origin, and redirects to `/:sessionId` so the deck opens in solo student mode.
- Compatibility constraints: `presentationUrl` must remain a public absolute `http(s)` URL that passes SyncDeck Reveal preflight from the ActiveBits origin. Hidden utility routes may be declared in `activity.config.*` with a `utilities[]` entry that omits `surfaces` so it is routable but not shown on `/manage` or `/`.
- Date: 2026-03-24
