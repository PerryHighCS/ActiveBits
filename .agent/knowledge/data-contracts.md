...
- Date: 2026-09-01
- Surface: activity interface (video-sync manager)
- Contract: `VideoSyncManager` is route-param keyed and can be reused (not remounted) across a parameter-only session swap. A `[sessionId]` effect resets `latestStateRef`, `state`, `telemetry`, and `setupMode` to their session-empty defaults before the new session's snapshot/websocket frames arrive, so the freshness guard (`shouldApplyIncomingVideoSyncState`) compares the new session's `serverTimestampMs` against its own baseline instead of the previous session's. Separately, the player-lifecycle effect reads `scheduleManagerPlaybackIntentFlush` through a ref (`scheduleManagerPlaybackIntentFlushRef`) instead of listing it as a direct effect dependency, because that callback's identity churns whenever `hasManagerAccess` flips during a 401/403 `revalidateManagerAccess()` cycle; without the ref indirection, an access-cookie refresh tore down and rebuilt the YouTube player and discarded any in-flight bounded playback-command retry.
