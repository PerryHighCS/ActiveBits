...
- Contract: `StudentSessionSnapshot` now includes `selfPacedMode: boolean`. When Resonance is launched from a standalone SyncDeck parent, the student snapshot treats the session as self-paced: `activeQuestions`/`activeQuestionIds` fall back to the full question set when no instructor-activated run exists, and once the viewer has submitted every question the snapshot synthesizes MCQ reveal entries with `correctOptionIds` and `viewerResponse` even though no instructor explicitly shared results.
