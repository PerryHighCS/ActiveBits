...
- Date: 2026-09-01
- Surface: activity interface (video-sync manager)
- Contract: `VideoSyncManager` is route-param keyed and can be reused (not remounted) across a parameter-only session swap. A `[sessionId]` effect resets `latestStateRef`, `state`, `telemetry`, and `setupMode` to their session-empty defaults before the new session's snapshot/websocket frames arrive, so the freshness guard (`shouldApplyIncomingVideoSyncState`) compares the new session's `serverTimestampMs` against its own baseline instead of the previous session's. `fetchSession` is passed the session-fetch effect's `AbortController` signal and drops an aborted response, so a still-pending snapshot request for the previous session cannot call `applyManagerStateUpdate` after the baseline reset (which would either restore the old session into the new view or leave the new session's legitimately older snapshot failing the freshness guard). Separately, the player-lifecycle effect reads `scheduleManagerPlaybackIntentFlush` through a ref (`scheduleManagerPlaybackIntentFlushRef`) instead of listing it as a direct effect dependency, because that callback's identity churns whenever `hasManagerAccess` flips during a 401/403 `revalidateManagerAccess()` cycle; without the ref indirection, an access-cookie refresh tore down and rebuilt the YouTube player and discarded any in-flight bounded playback-command retry.
- Compatibility constraints: Any new player-lifecycle effect dependency that is itself derived from `sendCommand`/`hasManagerAccess` should go through the same latest-ref pattern rather than being added to the effect's dependency array, unless a full player rebuild on every access-cookie refresh is actually intended.
