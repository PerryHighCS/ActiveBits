...
- Validation rules: Student clients reject a same-question session snapshot whose run start is older than the one already rendered, and same-run snapshots must not overwrite a local draft.
