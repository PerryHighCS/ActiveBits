...
- Contract: Teacher-led answer and draft writes identify an activation with the monotonic `activeQuestionRunRevision`; persisted drafts retain that revision and deadline finalization only promotes drafts from the current revision. Deadline finalization derives its transition through `SessionStore.updateAtomic`, so it operates on the newest record and cannot overwrite a concurrent draft commit from a detached timeout snapshot. At expiry, standard runs then clear active questions, while staged runs retain the expired current question and deadline until the instructor advances. Revision-less persisted drafts use the timestamp fallback only for migrated revision-1 runs. Self-paced sessions intentionally use a null run revision and null start timestamp, and the server accepts that null/null token before separately checking that the requested question is available. A `resonance:update-draft` carrying a bounded `draftId` receives `resonance:draft-saved { draftId }` after its write persists, or immediately without writing only when a confirmed response for that question/student in the current run has an equal-or-higher `editSequence` (the stale pre-submission cross-transport case); a higher-sequence revisit draft persists. A failed or unacknowledged draft save is retained at the parent (`ResonanceStudent`'s `QuestionDraftState` map, not the remounting `QuestionView`) and retried on a 1-second loop for as long as the activity stays mounted and the run remains current — not only after a reconnect. That retry stops when the draft (or a newer generation) is acknowledged as persisted, when a successful submission supersedes it, or when a confirmed response at an equal-or-higher edit sequence makes it discardable; it is reconciled from the server once the run's deadline has passed. Separately, if the socket itself is unavailable when a send is attempted, the session hook also queues that payload per question/run and replays it once reconnect confirms the same active, unexpired run — a transport-level retry underneath the parent's own domain-level one. Clients reconcile an unacknowledged deadline-window draft from the student snapshot. The student client also keys reactivation clearing of its submitted lock by revision, with the start timestamp retained only as a legacy fallback.
...
- Follow-up action: Resolved by PR #375 (closes #374). Unconfirmed-draft tracking was lifted to the parent: `ResonanceStudent` now owns a parent-level `Map<questionId, QuestionDraftState>` (survives `QuestionView` remounting on stack-tab switch) plus a 1-second retry loop, so a failed/unacknowledged `saveDraft()` before the deadline is retried and reconciled at the activity-lifetime level instead of being lost when the child unmounts. See `ARCHITECTURE.md`'s Resonance section and `.agent/plans/resonance-draft-state-consolidation.md` for the full design.
- Owner: Claude
