...
- Owner: Codex
- Contract: SyncDeck static presentation hosting should use the same-origin utility route `/util/syncdeck/launch-presentation?presentationUrl=<absolute-url>` rather than browser-side cross-origin calls to `/api/syncdeck/*`. The presentation supplies only `presentationUrl`; ActiveBits runs SyncDeck preflight, creates/configures the session on its own origin, stores the instructor bootstrap state locally, and redirects to `/manage/syncdeck/:sessionId`.
