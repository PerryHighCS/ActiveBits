...
- Validation rules: A pending retry timer (`playbackCommandFlushTimerRef`) and `desiredPlaybackIntentRef` must survive a `hasManagerAccess` flap; only an explicit `setupMode` transition or `sessionId` change may destroy the player instance.
