...
- Follow-up action: A related, deliberately unfixed gap remains in `activities/resonance/client/student/QuestionView.tsx` (~line 155, flagged by Copilot on PR #372): a failed/unacknowledged `saveDraft()` before the deadline only sets a ref with no retry, and if the student switches stack tabs the sole `QuestionView` for that question unmounts before its `disabled` effect can call `onDraftUnconfirmed`, so the server never learns about that draft. A safe fix needs unconfirmed-draft tracking lifted to the parent/hook level (real activity-lifetime lifecycle to bound retries), not a local retry loop in this component — a local retry can't reliably detect a post-unmount run/question change via frozen refs and risks an unbounded per-abandoned-question timer if the socket stays down.
- Owner: Claude
