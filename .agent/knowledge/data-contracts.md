...
- Validation rules: Student clients reject a same-question session snapshot whose run start is older than the one already rendered, but reset snapshot ordering state when either the session ID or student ID changes. Transitioning from idle/self-paced mode into a live run is a new run even when its question IDs are unchanged, so retained local answers for those questions must be cleared. Same-run snapshots must not overwrite a local draft. Draft-update messages are bound to the connected socket's student ID; a payload cannot select another student record.
