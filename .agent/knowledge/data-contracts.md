...
- Evidence (schema/tests/path): `types/activity.ts`; `types/activityConfigSchema.ts`; `server/activityConfigSchema.test.ts`; `ARCHITECTURE.md`; `ADDING_ACTIVITIES.md`; `activities/syncdeck/server/routes.ts`; `activities/syncdeck/server/routes.test.ts`; `activities/syncdeck/client/manager/SyncDeckManager.tsx`; `activities/syncdeck/client/manager/SyncDeckManager.test.tsx`; `activities/gallery-walk/activity.config.ts`; `activities/gallery-walk/server/reportHtml.ts`; `activities/gallery-walk/server/reportHtml.test.ts`; `activities/gallery-walk/server/reportRoute.test.ts`
- Follow-up action: Add a structured child-report data endpoint alongside the current standalone child HTML route, then teach SyncDeck to build a `SyncDeckSessionReportManifest` from all embedded child sessions.
- Owner: Codex
...
- Contract update: generic persistent-link URLs now carry signed query state for permalink meaning. `POST /api/persistent-session/create` returns `/activity/:activity/:hash?...&entryPolicy=...&urlHash=...`, and generic permalink entry/auth routes trust verified URL state for `entryPolicy` before any runtime metadata. Teacher cookies persist the same `entryPolicy` + `urlHash` so the dashboard can reconstruct links after a restart.
