...
- Compatibility constraints: Server frames must keep carrying a finite, monotone-per-mutation `serverTimestampMs` (`normalizeState` backfills `Date.now()` when absent/invalid). The heartbeat re-stamps `serverTimestampMs` on every tick, so cross-instance freshness has two layers: (1) the heartbeat takes a strict (cache-bypassing) Valkey read for its snapshot and, on a persisting tick, re-reads strictly and broadcasts the newer state if another instance committed one; (2) the client `shouldApplyIncomingVideoSyncState` guard is the accepted backstop for the residual read-to-publish window and for late/duplicate/reordered frames. A storage-level session revision / compare-and-set primitive is deferred cross-activity work.
...
- Contract: The manager mutes only its own programmatic `onStateChange` echo, not every event inside the suppression window. `resolveManagerStateChangeIntent({ suppressed, nextIntent, programmaticTarget })` records an intent unless it is `null` or (`suppressed` and equal to the `play`/`pause` transition `applyStateToPlayer` just requested, tracked in `programmaticPlaybackTargetRef`). An opposite-direction instructor click inside the window is still recorded and flushed. A recorded intent is additionally only sent when the manager document has recent genuine user activation - `isManagerPlaybackGestureRecent({ userActivationSupported, msSinceLastUserActivation })`, driven by `navigator.userActivation.isActive`. Per the HTML Standard's activation notification steps, a user interaction propagates its "last activation timestamp" to the interacted document **and every ancestor navigable regardless of origin** (the same-origin filter applies only to descendants), so a click on the control bar inside the cross-origin YouTube iframe does activate the ActiveBits parent document; the `onStateChange` that YouTube then posts back arrives well inside the transient-activation window, and `MANAGER_USER_GESTURE_GRACE_MS` (plus `lastUserActivationAtRef`, refreshed on every activated event) bridges the postMessage latency. Browsers without `navigator.userActivation` keep the prior mirror-always behavior. This stops connected managers whose player cannot reach the shared state (autoplay-blocked / mid-seek / buffering) from echoing involuntary pauses back as commands. A natural end-of-video `ENDED` is exempt from the gesture-recency gate (`shouldMirrorManagerPlaybackTransition({ isNaturalCompletion, gestureRecent })`), since it always lands long after the last click but must still reach the server or the session stays `isPlaying: true` and heartbeats drive an end-of-video replay loop.
- Compatibility constraints: `flushManagerPlaybackIntent` still no-ops a flush whose intent already matches authoritative state, so a recorded echo costs nothing. `sendCommand` returns a `SendCommandResult` and `resolveManagerPlaybackFlushOutcome` retains+retries the intent (bounded by `MAX_MANAGER_PLAYBACK_FLUSH_RETRIES`, `MANAGER_PLAYBACK_COMMAND_RETRY_DELAY_MS` apart) **only for a confirmed 401/403 (or a still-resolving capability)**; a permanent 4xx/5xx or an ambiguous network/parse failure is dropped, never replayed, because a `play`/`pause` is not idempotent and a replay at the original position could rewind the session (the next heartbeat/state-update reconciles instead). The manager surfaces an autoplay-blocked "Click to start playback" affordance (`autoplayBlocked`) like the student view when a programmatic play is refused; `retryManagerAutoplay` re-applies authoritative state through `applyStateToPlayer` (seek to the projected position + arm the suppression window / programmatic target) rather than a bare `playVideo()`, so the recovery does not rewind other clients to this lagging player's position.
- Evidence (schema/tests/path): `activities/video-sync/client/manager/VideoSyncManager.tsx`; `activities/video-sync/client/manager/VideoSyncManager.test.ts`; `.agent/knowledge/repo_discoveries.md` (2026-09-01 multi-manager feedback loop)
- Follow-up action: If the YouTube API begins emitting reliable seek events, fold seek intent into the same recorder rather than adding a parallel suppression path. If multiple concurrent managers become common, prefer a server-assigned control token over per-manager mirroring.
- Owner: Claude

- Date: 2026-09-01
- Surface: activity interface (video-sync manager)
- Contract: `VideoSyncManager` is route-param keyed and can be reused (not remounted) across a parameter-only session swap. A `[sessionId]` effect resets `latestStateRef`, `state`, `telemetry`, `setupMode`, `sourceUrlInput`, and `errorMessage` to their session-empty defaults before the new session's snapshot/websocket frames arrive (`stopSecInput`/`hasStopTime` follow `state.stopSec` via their own effect), so the freshness guard (`shouldApplyIncomingVideoSyncState`) compares the new session's `serverTimestampMs` against its own baseline and the setup form never shows or Saves the previous session's URL/stop time. Every state-producing async response is bound to the session it was issued for: `fetchSession` is passed the session-fetch effect's `AbortController` signal, and `sendCommand` / `saveConfigWithValues` check `sessionIdRef.current !== sessionId` after **every** `await` - after the `fetch`, after the failure-body parse (before `revalidateManagerAccess` / the error banner), after the success-body parse, and as the first line of the `catch`. None of them can call `applyManagerStateUpdate`, `setErrorMessage`, or `revalidateManagerAccess` for a previous session after the `[sessionId]` baseline reset (which would either restore the old session into the new view — the empty baseline accepts any frame — or leave the new session's legitimately older snapshot failing the freshness guard). The websocket path is already covered by the `envelope.sessionId !== sessionId` filter plus reconnect-on-`sessionId`-change. Separately, all self-rescheduling playback flushes go through `flushManagerPlaybackIntentRef` (kept pointed at the latest `flushManagerPlaybackIntent` by an effect), so a retry timer armed while `hasManagerAccess` was momentarily `false` during a `revalidateManagerAccess()` cycle re-runs the current callback (which sees access restored) rather than a stale closure bound to `hasManagerAccess === false`. This also makes `scheduleManagerPlaybackIntentFlush` referentially stable (its only dependency is a `[]`-stable callback), so the player-lifecycle effect can depend on it directly without the access-cookie refresh churn tearing down and rebuilding the YouTube player (which would also wipe an in-flight retry).
...
- Validation rules: A pending retry timer (`playbackCommandFlushTimerRef`) and `desiredPlaybackIntentRef` must survive a `hasManagerAccess` flap; only an explicit `setupMode` transition or `sessionId` change may destroy the player instance. `flushManagerPlaybackIntent` captures the intent+position it sent and, if `desiredPlaybackIntentRef`/`desiredPlaybackPositionRef` changed during the `await` (a newer gesture or a session swap took ownership), it never clears them - it schedules a follow-up flush so the newer intent is not dropped by a failed/older send.
