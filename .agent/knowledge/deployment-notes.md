...
- Additional strict reads: the `/command` handler, the `GET /api/video-sync/:sessionId/session` projection, and the websocket initial-snapshot path (`ensureVideoSyncSessionData` -> `applyStopIfReached` -> `sessions.set`) also use `{ strict: true }`. `applyStopIfReached` re-stamps `serverTimestampMs` to now, so any of those cache-backed reads could otherwise persist / broadcast / hand a reconnecting client a freshly stamped pre-pause `isPlaying:true` that the client freshness guard would accept over the authoritative paused state. On a Valkey outage the `/command` and `/session` routes answer a retryable 500 and the socket's initial snapshot is skipped (structured log `event: initial-snapshot-failed`).
