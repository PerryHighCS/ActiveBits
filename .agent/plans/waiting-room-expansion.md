...
- `syncdeck`: substantially converged. It now collects `displayName` in the shared waiting room and requires accepted-entry identity before the activity-specific registration-plus-reconnect split can proceed. Full websocket-level adoption of the shared accepted-entry connect service is still deferred to the broader SyncDeck/presentation track.
