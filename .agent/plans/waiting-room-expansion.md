...
- close the remaining waiting-room test gap
  Helper and route coverage are strong, `WaitingRoomContent.tsx` now gives us render-level accessibility/control-state coverage without importing the Vite activity loader, and `waitingRoomHandoffUtils.ts` now gives us direct carry-forward persistence coverage for the server-backed token-vs-local-fallback branch. The remaining gap is full interaction coverage for `WaitingRoom.tsx` websocket state and end-to-end form submission.
...
- [ ] Add tests for required-field blocking, validation behavior, accessibility-critical control states, and wait-to-entry state carry-forward
  Current status: utility-level coverage now exists for waiting-room field validation/storage (`waitingRoomFormUtils.test.ts`), primary-action blocking rules (`waitingRoomActionUtils.test.ts`), live-session entry-token storage/consume behavior including 404 vs retry semantics (`entryParticipantStorage.test.ts`), server-backed-vs-local carry-forward persistence in `WaitingRoom` itself (`waitingRoomHandoffUtils.test.ts`), shared live-entry/join/solo/wait/pass-through status resolution (`server/entryStatus.test.ts`), and render-level accessibility/control-state coverage through `WaitingRoomContent.test.tsx`. Full interaction coverage for websocket-driven state and full end-to-end `WaitingRoom.tsx` submission flow is still outstanding, but the newer seams avoid the previous hard stop from the Vite activity-loader path.
  Follow-up target: first add one more stable component-level seam around `WaitingRoom.tsx` websocket/wait-state transitions without `import.meta.glob`; only revisit a browser-level harness after those cheaper seams are exhausted and a real gap remains.
