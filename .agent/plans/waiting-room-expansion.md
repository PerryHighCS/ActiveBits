...
- [ ] Define shared server-side accepted-entry / reconnect semantics around `participantId`
  ...
  Update: `java-string-practice`, `java-format-practice`, `traveling-salesman`, and now the Python List Practice server routes also share session-backed participant semantics instead of route-local matching. Waiting-room handoff issues `participantId` earlier through shared entry-participant storage before downstream activity startup, the migrated Java activities share one client-side post-handoff identity hydration helper before websocket join, and shared server-side participant helpers now cover accepted-participant lookup, disconnect handling, and later state updates (`lastSeen`, progress/route submission) across those shared-path activities. Session entry-token consume now also records accepted participant identity server-side in `acceptedEntryParticipants`, and `connectAcceptedSessionParticipant(...)` now preserves that accepted `participantId` on first create instead of minting a fresh ID. It also now supports ID-only reconnect for existing participants by reusing the already-stored participant name, which lets the Java activities rejoin cleanly even when only the browser's stored `participantId` survives. SyncDeck now uses that same direct accepted-entry connect model for new student websocket joins: it declares a required waiting-room `displayName` field, the student client carries accepted identity straight into websocket connect, and the server can create the first SyncDeck student record from accepted entry without a separate pre-connect registration hop.
  Next step: promote session-scoped reconnect identity out of activity-local startup code and into the shared entry layer itself. The waiting-room/entry system should persist reusable accepted participant context for a session as soon as entry is accepted, so reload and rejoin do not depend on each activity remembering to write `student-name-*` / `student-id-*` at the right time.
  Remaining gap: the project still does not have one fully authoritative accepted-entry service across every activity type and later mutation surface.
