...
- [ ] Define shared server-side accepted-entry / reconnect semantics around `participantId`
  ...
  Update: `java-string-practice`, `java-format-practice`, `traveling-salesman`, and now the Python List Practice server routes also share session-backed participant semantics instead of route-local matching. Waiting-room handoff issues `participantId` earlier through shared entry-participant storage before downstream activity startup, the migrated Java activities share one client-side post-handoff identity hydration helper before websocket join, and shared server-side participant helpers now cover accepted-participant lookup, disconnect handling, and later state updates (`lastSeen`, progress/route submission) across those shared-path activities. Session entry-token consume now also records accepted participant identity server-side in `acceptedEntryParticipants`, and the new `connectAcceptedSessionParticipant(...)` service lets `java-string-practice`, `java-format-practice`, `traveling-salesman`, and `python-list-practice` websocket joins recover the accepted-entry display name by `participantId` when the reconnecting client no longer sends `studentName`. SyncDeck now partially converges on that same model from the registration side: its `register-student` route can reuse accepted-entry `participantId` plus waiting-room display name, and the student client can auto-register from accepted waiting-room identity instead of forcing a second manual submit or re-showing the manual join form while that accepted identity is being finalized. The remaining gap is narrower now: SyncDeck websocket connect still remains reconnect-only and does not use the shared accepted-entry connect service directly, and the project still does not have one fully authoritative accepted-entry service across activity types and later mutation surfaces.
...
- `syncdeck`: partially converged. Student identity still keeps its activity-specific registration-plus-reconnect split, but waiting-room accepted-entry identity now flows into that registration step and can auto-complete it. Full websocket-level adoption of the shared accepted-entry connect service is still deferred to the broader SyncDeck/presentation track.
