...
- [ ] Define shared server-side accepted-entry / reconnect semantics around `participantId`
  ...
  Update: `java-string-practice`, `java-format-practice`, `traveling-salesman`, and now the Python List Practice server routes also share session-backed participant semantics instead of route-local matching. Waiting-room handoff issues `participantId` earlier through shared entry-participant storage before downstream activity startup, the migrated Java activities share one client-side post-handoff identity hydration helper before websocket join, and shared server-side participant helpers now cover accepted-participant lookup, disconnect handling, and later state updates (`lastSeen`, progress/route submission) across those shared-path activities. Session entry-token consume now also records accepted participant identity server-side in `acceptedEntryParticipants`, which narrows the remaining gap further: the project no longer lacks a server-side accepted-entry record, but it still lacks one broader service that makes activity join/reconnect flows consistently authoritative against that record.
