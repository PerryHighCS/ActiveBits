...
- [x] Define shared server-side accepted-entry / reconnect semantics around `participantId`
  Current status: the shared contract is now effectively complete for the session-backed student-participant family (`java-string-practice`, `java-format-practice`, `python-list-practice`, `traveling-salesman`, and SyncDeck student identity). Waiting-room handoff issues `participantId` earlier through shared entry-participant storage, `acceptedEntryParticipants` records accepted identity server-side, `connectAcceptedSessionParticipant(...)` preserves accepted IDs on first create and supports ID-only reconnect, and shared participant helpers now cover accepted lookup, disconnect handling, and later state updates (`lastSeen`, progress/route submission) across the shared-path activities.
  Deferred boundary: this is not a universal participant service for every activity type. Activities with substantially different identity/session models (`gallery-walk`, `raffle`, `www-sim`) remain intentionally outside this shared contract unless they later adopt waiting-room entry in a way that genuinely fits it.
  Follow-on work: keep migrating session-scoped reconnect identity persistence into the shared entry layer where it clearly helps, but treat broader cross-activity unification as a separate product/design decision rather than an open blocker for waiting-room completion.
...
- [x] Document destination transitions for `wait`, `join-live`, `continue-solo`, and `solo-unavailable`
  Update: `continue-solo` permalinks no longer auto-redirect away from the waiting room. When no live session is active, the waiting room remains visible so students can choose solo entry and instructors without a remembered cookie still have a place to enter their teacher code and start live.
- [x] Document presentation-mode rules for `render-ui` vs `pass-through`
- [x] Define how permalink and ad-hoc join-code entry both route through the same waiting-room flow
  Current status: direct `/:sessionId` and permalink `/activity/:activityName/:hash` entry both ask the server for an entry-status payload before the router decides whether to wait, preflight, join live, or continue solo. Those payloads are assembled through a shared server-side entry-status builder, and both entry-participant handoff flows share backend token/normalization rules. Separate `sessionId` and `hash` entrypoints are now considered an acceptable end state unless a later operational reason appears to unify lookup + handoff orchestration behind one gateway service.
...
- [x] Retain preflight form state across destination transitions (for example `wait -> join-live`) and carry collected data forward when entry proceeds
  Current status: started persistent sessions with waiting-room fields now stay inside the same `WaitingRoom` shell and reuse the stored preflight form state for `join-live`. Both live-session entry and persistent permalink `continue-solo` can now store waiting-room values server-side behind opaque handoff tokens before entry proceeds, with client storage preserved as fallback.
- [x] Submit/store preflight data for later entry flow use
  Current status: waiting-room `join-live` posts collected values to a server-backed session entry-participant store, and permalink `continue-solo` posts collected values to a parallel persistent-session entry-participant store. Both paths leave only opaque tokens in client storage when the server-backed write succeeds, with client-value fallback preserved if storage fails. `java-string-practice` and `java-format-practice` both consume `displayName` and a shared server-issued `participantId` from the handoff path to skip duplicate startup prompts and reconnect earlier. Broader activity adoption and a fully shared participant context are still outstanding follow-up work, not missing framework capability.
- [x] Add clear [TEST] logging for expected error-path tests
  Current status: the waiting-room-specific noisy-path coverage now uses explicit `[TEST]` markers where needed, including the corrupted-cookie parse path in `server/persistentSessionRoutes.test.ts`. Further `[TEST]` markers can be added opportunistically in future tests, but this is no longer an active waiting-room-core gap.
- [x] Add tests for required-field blocking, validation behavior, accessibility-critical control states, and wait-to-entry state carry-forward
  Current status: utility-level coverage now exists for waiting-room field validation/storage (`waitingRoomFormUtils.test.ts`), primary-action blocking rules (`waitingRoomActionUtils.test.ts`), websocket message transition decisions (`waitingRoomTransitionUtils.test.ts`), cookie-backed teacher auto-auth on websocket open (`waitingRoomAutoAuthUtils.test.ts`), post-auth teacher-code next-step decisions (`waitingRoomTeacherSubmitUtils.test.ts`), websocket handler wiring across open/message/error/close (`waitingRoomSocketUtils.test.ts`), live-session entry-token storage/consume behavior including 404 vs retry semantics (`entryParticipantStorage.test.ts`), server-backed-vs-local carry-forward persistence in `WaitingRoom` itself (`waitingRoomHandoffUtils.test.ts`), shared live-entry/join/solo/wait/pass-through status resolution (`server/entryStatus.test.ts`), and render-level accessibility/control-state coverage through `WaitingRoomContent.test.tsx`. Remaining coverage beyond this point is true container-level form/network integration, which is tracked separately as a tooling/value decision rather than a missing logic-test box.
...
- [ ] Ensure embedded entry inherits role from parent context and does not prompt for instructor code
  Current status: there is still no child activity launcher on the shared waiting-room path yet, but SyncDeck now exposes a first parent-context proof surface at `POST /api/syncdeck/:sessionId/embedded-context`. That endpoint can validate inherited `teacher` role from instructor passcode or inherited `student` role from a registered SyncDeck `studentId`, which narrows the remaining gap to wiring a child launch path onto that validated context instead of inventing inheritance rules in the child.
- [x] Add tests for role resolution, live join, wait, solo fallback, pass-through, unsupported-solo cases, and direct-API bypass attempts
  Current status: shared server-side entry-status matrix tests now cover live join, wait, solo fallback, pass-through, and unsupported-solo outcomes in `server/entryStatus.test.ts`, and route-level coverage in `server/sessionEntryRoutes.test.ts` plus `server/persistentSessionRoutes.test.ts` includes missing-session handling, token-trimming/blank-token behavior, malformed permalink entry requests, corrupted-cookie handling, stale backing-session repair, student-vs-teacher live-entry role boundaries, and solo-unavailable permalink outcomes. Remaining coverage questions are explicitly deferred to embedded-role inheritance and any future higher-level browser/container harness decision.
- [x] Add a test proving `solo-only` links with instructor auth resolve to `continue-solo`, not `join-live`

### Phase 4 - Follow-on Activity Migration Track

Status note:

- Phase 4 is now a follow-on track, not an active waiting-room-core checklist.
- The remaining core waiting-room work is now mostly limited to embedded role inheritance plus any later decision to add browser-level interaction coverage.
- Existing migration notes below should be treated as rollout guidance and cleanup planning, not as blockers for calling the waiting-room core substantially complete.
...
- [x] Verify `java-string-practice` reconnect/progress flows still work with waiting-room-provided `participantId`
...
- [ ] Move session-scoped reconnect identity persistence into the shared entry layer instead of activity-local startup code
  Current status: a new shared session-participant context helper now exists on the client side, and the Java activities read/write through it instead of relying only on raw `student-name-*` / `student-id-*` keys. The remaining work is to make entry acceptance itself the authoritative writer for that context across more activities and remove the leftover compatibility writes.
...
- `video-sync`: partially converged. It now declares the shared waiting-room `displayName` field and resolves a stable student `participantId` through shared entry/session identity helpers for telemetry and reconnect-friendly local context, while keeping its websocket/session read surfaces activity-owned.
...
- `syncdeck`: substantially converged. It now collects `displayName` in the shared waiting room and uses accepted-entry identity directly on websocket connect for new student entry. The main remaining divergence is the activity-owned route/protocol surface, not duplicate identity collection.
