...
- [ ] Define shared server-side accepted-entry / reconnect semantics around `participantId`
  ...
  Update: `java-string-practice`, `java-format-practice`, `traveling-salesman`, and now the Python List Practice server routes also share session-backed participant semantics instead of route-local matching. Waiting-room handoff issues `participantId` earlier through shared entry-participant storage before downstream activity startup, the migrated Java activities share one client-side post-handoff identity hydration helper before websocket join, and shared server-side participant helpers now cover accepted-participant lookup, disconnect handling, and later state updates (`lastSeen`, progress/route submission) across those shared-path activities. Session entry-token consume now also records accepted participant identity server-side in `acceptedEntryParticipants`, and the Java websocket join paths now consult that accepted-entry name by `participantId` when the reconnecting client no longer sends `studentName`. The remaining gap is that this is still an incremental rule layered onto activity-owned join/reconnect flows, not one broader accepted-entry service across activity types.
