...
- close the remaining waiting-room test gap
  Helper and route coverage are strong, `WaitingRoomContent.tsx` now gives us render-level accessibility/control-state coverage without importing the Vite activity loader, `waitingRoomHandoffUtils.ts` now gives us direct carry-forward persistence coverage for the server-backed token-vs-local-fallback branch, `waitingRoomTransitionUtils.ts` now gives us direct coverage for websocket message transition decisions, `waitingRoomAutoAuthUtils.ts` covers cookie-backed teacher auto-auth on socket open, and `waitingRoomTeacherSubmitUtils.ts` covers the post-auth teacher-code branch between manage redirect, websocket verification, and failure states. The remaining gap is full interaction coverage for the websocket lifecycle wiring and true container-level form/network integration.
...
- [ ] Define shared server-side `participantId` issuance and reconnect semantics
  ...
  Update: `java-string-practice`, `java-format-practice`, `traveling-salesman`, and now the Python List Practice server routes also share session-backed participant semantics instead of route-local matching. Waiting-room handoff issues `participantId` earlier through shared entry-participant storage before downstream activity startup, the migrated Java activities share one client-side post-handoff identity hydration helper before websocket join, and shared server-side participant helpers now cover accepted-participant lookup, disconnect handling, and later state updates (`lastSeen`, progress/route submission) across those shared-path activities. SyncDeck now also shares the duplicate-socket rule plus an activity-owned wrapper for websocket student reconnect/touch behavior, and its websocket path now rejects missing or unknown student IDs instead of silently accepting client-invented ones. REST registration and embedded-role authority are still activity-owned, and the project still lacks one authoritative “accepted participant entry” contract after handoff.
...
- [ ] Add tests for required-field blocking, validation behavior, accessibility-critical control states, and wait-to-entry state carry-forward
  Current status: utility-level coverage now exists for waiting-room field validation/storage (`waitingRoomFormUtils.test.ts`), primary-action blocking rules (`waitingRoomActionUtils.test.ts`), websocket message transition decisions (`waitingRoomTransitionUtils.test.ts`), cookie-backed teacher auto-auth on websocket open (`waitingRoomAutoAuthUtils.test.ts`), post-auth teacher-code next-step decisions (`waitingRoomTeacherSubmitUtils.test.ts`), live-session entry-token storage/consume behavior including 404 vs retry semantics (`entryParticipantStorage.test.ts`), server-backed-vs-local carry-forward persistence in `WaitingRoom` itself (`waitingRoomHandoffUtils.test.ts`), shared live-entry/join/solo/wait/pass-through status resolution (`server/entryStatus.test.ts`), and render-level accessibility/control-state coverage through `WaitingRoomContent.test.tsx`. What remains untested is mostly the container wiring across websocket lifecycle events plus full end-to-end submission/network integration, not the underlying decision matrices.
  Follow-up target: do not add more tiny helper seams just to chase 100% isolation. If deeper coverage is still desired after this point, reassess whether the remaining value justifies a higher-level interaction harness.
