...
- [x] Known gap from Slice A: Java Format's permalink / persistent-teacher entry bypasses `POST /create`, so no manager capability is issued and that flow would 403 / close `1008`. **Interim stopgap applied:** `activity.config.ts` now sets `supportsPermalink: false` for `java-format-practice`. Note this is a single coarse flag (`activitySupportsStandalonePermalink`) that also gates **solo** permalinks (`solo-only` / `solo-allowed` persistent links), so those are disabled for Java Format too until the flag is restored — an accepted temporary regression; solo play via the home card / direct path is unaffected. Re-enable it as part of the Phase 6 wrap-up once the shared persistent-teacher to manager-principal adapter (proven in Slice C) issues the capability. Tracked in [#351](https://github.com/PerryHighCS/ActiveBits/issues/351). (Wave 1 practice activities share this permalink config and should inherit the settled pattern.)
