...
- Parent writes: solo start, embedded-activity start, return-to-waiting-room, and parent deletion share one in-process lock per parent (`buildParentWriteLockKey`); deletion reads the parent under the lock, so its cascade includes any child a finished start bound. Solo start applies only its `soloChildren` change to a fresh read of the parent just before writing, so it never writes back fields another writer changed. Other SyncDeck parent writers (WebSocket roster, configure) and cross-instance writes still use plain `set()` and remain #313's scope.
