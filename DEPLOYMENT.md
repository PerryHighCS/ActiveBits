...
7. **Video Sync unsynced-student telemetry**: In Valkey mode, `video-sync` stores per-session unsynced-student timestamps in a Valkey-backed key (with short TTL pruning) so `telemetry.sync.unsyncedStudents` stays coherent when `/api/video-sync/:sessionId/event` requests land on different instances. In in-memory mode this telemetry remains single-instance only, which is acceptable for local/dev deployments.
   The `video-sync` 3s heartbeat, the `/command` handler, the `GET /api/video-sync/:sessionId/session` projection, and the websocket initial-snapshot path all read the session with a strict (cache-bypassing) Valkey GET so a multi-instance deploy cannot rebroadcast, hand a reconnecting client, or persist a stale `isPlaying:true` from a peer's local cache after another instance handled a pause (`applyStopIfReached` re-stamps `serverTimestampMs` to now, so a stale projected frame would otherwise look newer than the real paused state). This adds ~1 Valkey GET per session per 3s while subscribers are connected, plus one per `/session` GET and per socket connect; on a Valkey outage those reads reject — a heartbeat tick is skipped (logged), the HTTP routes answer a retryable 500, and the socket's initial snapshot is skipped (logged) — rather than serving stale local state. Clients additionally drop any incoming state frame whose `serverTimestampMs` is older than what they have applied.
