...
11. **Cross-origin SyncDeck manager bootstrap tokens**: Ad hoc SyncDeck promotion from an external presentation host now stores short-lived one-time manager bootstrap records on `session.data.managerBootstraps`. Keep those records in the shared session store so redirects and multi-instance deployments can redeem `/manage/syncdeck/:sessionId#bootstrap=...` on any instance that can read the canonical session record. The fragment-based handoff keeps the bootstrap token out of normal cross-origin `Referer` forwarding. Tokens are not long-lived credentials and should be pruned on consume/expiry.
