...
19. **SyncDeck embedded manager bootstrap timing**: SyncDeck embedded-start responses carry a short-lived, single-use `managerEntryToken` for the child manager. The parent must wait for this authenticated response before mounting the instructor iframe; the iframe exchanges the token at the same-origin SyncDeck endpoint for its child passcode, then replaces its URL to remove the consumed query token. This avoids the websocket-start race and does not persist credentials in browser storage. Ad-hoc sessions depend on this path because they do not have persistent teacher-cookie recovery.
