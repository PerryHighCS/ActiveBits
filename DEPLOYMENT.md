...
19. **SyncDeck embedded manager bootstrap timing**: SyncDeck embedded-start responses can carry child manager bootstrap data such as Video Sync's `instructorPasscode`. The manager must refresh an already-mounted embedded manager iframe after caching that bootstrap payload, because the websocket start broadcast can make the child render before the HTTP start response arrives. Ad-hoc sessions depend on this refresh path because they do not have persistent teacher-cookie recovery, and Video Sync must also retry bootstrap auto-start once delayed instructor credentials arrive instead of leaving the manager in a failed credential state.
