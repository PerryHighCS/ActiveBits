...
13. **Resonance timeout finalization**: When a timed active run expires, an in-process server deadline task promotes every valid server-persisted draft from that run into a submitted response and broadcasts the result. For standard runs, it then clears the active questions. Staged runs retain their expired active question and deadline so the instructor can advance the run deliberately. Loading a future-deadline session re-establishes this task after a restart or reconnect; normal session access remains a fallback expiration trigger. Long waits are segmented at Node's maximum timer delay, and a failed strict store read or persist re-arms the task instead of dropping finalization. Finalization mutates a detached session copy and only exposes it after persistence, so a failed write cannot make a local cache look expired and cancel the retry. Each run also carries a persisted monotonic revision: clients use it for snapshot ordering and the server requires it on current client draft/submission messages, so same-millisecond reactivation cannot admit work from the prior run. Student clients cap their draft debounce to flush shortly before the deadline and only regard a WebSocket draft as persisted after a correlated `resonance:draft-saved` server acknowledgement; after its final async mode lookup, the server rechecks the deadline and uses that same timestamp for a draft write, so deadline-crossing work is not acknowledged then discarded. An unacknowledged deadline-window draft is refreshed from the authoritative student snapshot. No client-supplied late-submit flag is trusted. `resonance:update-draft` is now the one Resonance mutation persisted through `SessionStore.updateAtomic(...)` (see item 15); every other Resonance mutation still uses whole-session writes, so horizontal scale-out safety still depends on migrating the rest under #313 before enabling multiple writers for one session.
14. **Resonance participant capabilities**: Resonance registration issues an opaque, httpOnly `activebits_cap_participant_<base64url(sessionId)>` cookie with the shared seven-day bounded server expiry. An already-authenticated registration reload reuses that capability rather than minting another record. New registration uses the non-evicting shared issuer: expired records are reclaimed, but capacity returns 429 instead of invalidating another live participant. Unauthenticated direct-name registrations are additionally limited to 100 per minute per trusted-proxy client IP and session, accommodating a classroom behind one NAT while preserving half of the 200-record capability pool against one burst; accepted waiting-room participants and existing capabilities are not counted against that direct-entry bucket. When a shared browser presents both a stale capability and a fresh accepted waiting-room token, the accepted participant wins and receives a replacement capability; that handoff token is consumed in the same successful session write, so it cannot be replayed to mint more records. Registration stages those mutations on a detached record; if persistence fails, it logs a structured error and returns 503 without setting a cookie or consuming the handoff locally, so the participant can retry. A restored local student ID that receives 403 is discarded once and registration retries without that unauthenticated hint, producing a fresh direct identity or recovering the waiting-room identity. Student REST state/submission routes and WebSocket admission derive the student ID from that capability; query/body IDs are consistency checks only. REST authorization denials emit structured `student-answer-submit-denied` or `student-state-denied` warnings with only the session ID and a non-sensitive reason; capability values are never logged. Waiting-room IDs can be claimed only when the existing accepted-participant cookie proves the same identity, while direct entry receives a new server-generated ID. This is a clean cutover: pre-deployment live sessions without Resonance participant capabilities are not migrated. Preserve same-origin cookie forwarding, `path=/`, and production TLS so `Secure` cookies reach both REST and WebSocket upgrades.
15. **Resonance draft persistence and client-side retry**: `resonance:update-draft` requires `SessionStore.updateAtomic(...)`; on a store that doesn't expose it the handler logs a structured `resonance.draft-save-unavailable` error and refuses the write rather than falling back to an unsafe read-modify-write. Each draft carries a monotonic generation (allocated client-side, per question+run, by the student view — not the remounting `QuestionView` child) that the atomic mutator compares against the freshest stored generation and the freshest confirmed response before persisting, so a write that raced behind a newer one (client retry, reconnect replay, or a fresh submission) is acknowledged as a no-op instead of overwriting. On the client, a save that fails (send error or unacknowledged within the ack timeout) is retained by the parent student view, not the child `QuestionView` that failed to save it, since that child unmounts on every stack-tab switch; a 1-second interval retries it against the live session snapshot until it's superseded — a confirmed response at an equal-or-higher edit sequence (from a later snapshot), a successful retry of that same draft, a successful submission, or an acknowledged newer generation (see below); self-paced mode has no deadline and never removes a question from the active set, so these are its only ways to stop — or reconciled at the run's deadline in a live run. A successful submission also clears any retained autosave for that question immediately (rather than waiting on the next snapshot to carry a matching edit sequence), since submission is a REST call independent of the WebSocket and can succeed while the socket — and therefore the retry loop's usual exit — is still down; the parent guards against a still-in-flight (already-sent, not yet acknowledged) autosave from before the submission failing afterward and resurrecting a retained draft, since that draft's run/answer still matches. A newer generation's own successful save is reported back to the parent (`onDraftSaved`) so it can clear an older generation still retained/retried for the same question+run, rather than leaving it to keep resending a superseded attempt. `useResonanceSession` separately queues an unacknowledged/failed draft for replay on WebSocket reconnect, using the same generation watermark to avoid replaying an attempt already superseded by a newer one still in flight when the socket dropped; it also exposes `cancelDraftRetries(key, atLeastGeneration)` so the parent can cancel/supersede that queue directly once it independently learns (via a submission or a newer save) that a queued retry is stale — the hook has no other way to find out, since neither event goes through `saveDraft`/`queueDraftRetry`. That cancellation bound is capped at the highest generation actually allocated so far for the question+run (not an unbounded sentinel): live Resonance allows revisiting a submitted question in the same run, and an unbounded cancel would make every later revisit edit's failed autosave permanently rejected from ever being queued for reconnect replay. A reconnect-replay ack is a separate, fire-and-forget path from a direct `saveDraft` ack — it doesn't resolve any caller-held promise — so the hook accepts an optional `onDraftReplayAcknowledged(questionId, runIdentity, generation)` callback and invokes it there — passing the retry's own resolved run identity, not just a composite key, so a late ack for a legacy (pre-revision) form can still be matched against a record already canonicalized to a revision number (see below) — letting the parent clear its own retained-draft entry for that specific replayed attempt instead of waiting on an unrelated retry to also get acknowledged. The hook also caps `cancelDraftRetries` internally, at the highest generation it has itself actually seen attempted for that key, rather than trusting the caller's value as-is — defense in depth on top of the parent's own bound, so a mistaken/unbounded caller can't permanently poison the watermark either. It tracks that cancellation ceiling separately from the "highest generation attempted" watermark used to reject stale sends: the two need different equality rules — a cancelled generation must block an exact-match retry (e.g. that same attempt's own ack timing out after it was superseded by a submission), while an ordinary in-flight attempt must still allow its own generation through once. A run restart drops any locally cached answer not stamped with the new run, so a stale prior-run value can't stay displayed or get resubmitted under the new run. Draft/submission answer comparisons (used to decide whether a retained draft matches its confirmed submission) treat MCQ `selectedOptionIds` as a set, not an ordered array — matching `QuestionView`'s own comparison — since click order can differ between a retained draft and its later-confirmed submission for the same selection. A retained draft canonicalized from its legacy (pre-revision) timestamp form to the revision-1 form can still have a reconnect-replay send already in flight under the legacy form when canonicalization happens; that send is untouched by the migration's own cancellation (which only reaches a still-*queued* hook entry), so its acknowledgement can arrive afterward still naming the legacy run identity. That late ack still resolves correctly — the parent's match check (`acknowledgeQuestionDraftGeneration`) compares the ack's own run identity against the local record via the same legacy-vs-canonical equivalence logic used everywhere else in Resonance's run-identity handling (`payloadMatchesResolvedRunToken`, `activities/resonance/shared/runIdentity.ts`), not a raw key match — so no separate alias table is needed to bridge the two forms. `QuestionView.submitAnswer`'s success path (`onSubmitted`) fires even after the view has unmounted (the student switched stack tabs before the REST response returned) — the parent, not this child, owns retained-draft/edit-sequence bookkeeping precisely because `QuestionView` remounts on every stack-tab switch, and would otherwise never learn a submission it can no longer directly observe had actually succeeded. Because it can fire well after unmount — and, since the parent may since have remounted a *replacement* view for the same question, possibly after further independent local edits — the parent never trusts the child's own (frozen, per-instance) checks or the closure this callback was created with: it re-derives current truth from `snapshotRef.current` and discards the response outright, before touching any state, if either (a) the run it belongs to is no longer the current one (a restart already handled by the ordinary run-restart cleanup, which this must not undo), or (b) the parent's current locally-cached answer for that question differs from the one this submission is about to apply (a replacement view has made its own newer, independent edit since). (b) was first tried as a comparison against a parent-owned draft-generation counter, which false-positived on this same view's own harmless flush-on-unmount re-send of the *same* answer it had just submitted (pending-debounce flush on unmount is a separate, pre-existing mechanism); comparing the submitted answer's content against the parent's current cached answer for that question has no such false positive. `recordUnconfirmedDraft` similarly compares an incoming failure's generation against the highest generation *attempted* so far for that question+run (not just the highest acknowledged, and not just what's already retained) before retaining it — a failure from an unmounted view can otherwise arrive after a replacement view's own newer save is already in flight (sent, not yet resolved either way), and get retained ahead of it. A live run's self-paced fallback (its session flips to self-paced mid-run, e.g. an embedding SyncDeck parent going standalone) purges any locally cached `submittedAnswers` entry still stamped with an actual run token before merging in the self-paced snapshot, the same way a live run restart does — self-paced mode has no run identity of its own, and this transition can arrive as the very next snapshot with no intermediate idle snapshot to trigger the ordinary run-restart cleanup. Server-side, the draft handler's freshness guard (finding the confirmed response for the current run, to reject a draft whose edit sequence hasn't advanced past it) uses a dedicated `responseMatchesActiveRun` matcher rather than strict equality on `activeQuestionRunRevision`: `normalizeSessionData` always upgrades the session-level run counter to at least 1 once a run is active, but `normalizeStoredResponses` never backfills that field on an individual stored response, so a response persisted before the revision rollout keeps it permanently `undefined` — strict equality would never recognize that response as belonging to the current (now revision-1) run, letting a stale pre-rollout draft slip past the guard and risk later promotion over the already-confirmed answer at deadline finalization.
