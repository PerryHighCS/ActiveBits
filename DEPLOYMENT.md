...
7. **Video Sync unsynced-student telemetry**: In Valkey mode, `video-sync` stores per-session unsynced-student timestamps in a Valkey-backed key (with short TTL pruning) so `telemetry.sync.unsyncedStudents` stays coherent when `/api/video-sync/:sessionId/event` requests land on different instances. In in-memory mode this telemetry remains single-instance only, which is acceptable for local/dev deployments.
   Every `video-sync` server path that reads the session and then persists it uses a strict (cache-bypassing) Valkey GET: the 3s heartbeat, the `/command` handler, the `GET /api/video-sync/:sessionId/session` projection, the websocket initial-snapshot path, and the telemetry-only writers (`POST /api/video-sync/:sessionId/event`, the unsynced-student prune timer, and websocket-disconnect cleanup). Otherwise a multi-instance deploy could rebroadcast, hand a reconnecting client, or persist a stale `isPlaying:true` from a peer's 30s local cache after another instance handled a pause (`applyStopIfReached` re-stamps `serverTimestampMs` to now, so a stale projected frame would otherwise look newer than the real paused state; the telemetry-only writers never re-stamp, so their residual sub-request race writes back a `state` whose older `serverTimestampMs` the client freshness guard rejects). This adds ~1 Valkey GET per session per 3s while subscribers are connected, plus one per `/session` GET, per `/event`, and per socket connect/disconnect; on a Valkey outage those reads reject — a heartbeat tick is skipped (logged), the HTTP routes answer a retryable 500, and the socket snapshot/cleanup is skipped (logged, self-healing on the next heartbeat) — rather than serving stale local state. Clients additionally drop any incoming state frame whose `serverTimestampMs` is older than what they have applied.
