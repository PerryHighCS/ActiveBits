...
20. **MobCode Brython runner popup**: The first MobCode runner opens a same-origin popup and loads Brython from jsDelivr before executing the selected Python entry file in the browser. Production environments that block third-party CDN scripts should either allow that Brython asset path or replace the runner with a self-hosted Brython bundle before relying on in-class execution.
