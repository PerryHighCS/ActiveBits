...
void test('handleReturnedToWaitingRoom clears identity and redirects only the targeted student', () => {
  ...
  assert.equal(handleReturnedToWaitingRoom({ participantId: 'ada', registeredStudentId: 'ada', sessionId: 's1', storage, sessionStorage: storage, redirect: (url) => { destination = url } }), true)
  assert.equal(destination, '/s1'); assert.ok(removed.includes('session-participant:s1'))
  assert.equal(handleReturnedToWaitingRoom({ participantId: 'lin', registeredStudentId: 'ada', sessionId: 's1', storage, sessionStorage: storage, redirect: () => {} }), false)
})
