...
void test('SyncDeckManager shows the active session id when provided', () => {
  ...
  assert.match(html, /aria-describedby="syncdeck-join-code-value"/i)
  assert.match(html, /id="syncdeck-join-code-value"/i)
  assert.match(html, /aria-label="Copy join URL"/i)
  assert.doesNotMatch(html, />Copy Join URL</i)
  assert.match(html, /Session Report/i)
  assert.ok(html.indexOf('Session Report') < html.indexOf('Join Code:'), 'Session Report should precede Join Code')
  assert.doesNotMatch(html, /Preview Report/i)
  assert.doesNotMatch(html, /Download Session Report/i)
