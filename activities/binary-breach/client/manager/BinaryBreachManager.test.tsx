...
function installDomEnvironment(url: string, sockets: TestSocket[] = []) {
...
function buildStateResponse(maxBits: 4 | 5 | 6 | 7 | 8) {
  return {
    settings: {
      maxBits,
      missionLength: 5,
...
void test('BinaryBreachManager keeps local edits when an older state fetch resolves', { concurrency: false }, async () => {
  const restoreDom = installDomEnvironment('https://bits.example/manage/binary-breach/session-one')
  const previousFetch = globalThis.fetch
  ...
  try {
    ...
    const rendered = render(
      <MemoryRouter initialEntries={['/manage/binary-breach/session-one']}>
        <Routes>
          <Route path="/manage/binary-breach/:sessionId" element={<BinaryBreachManager />} />
        </Routes>
      </MemoryRouter>,
    )
    unmount = rendered.unmount

    const maxBitsSelect = rendered.getByLabelText(/maximum bits/i) as HTMLSelectElement
    fireEvent.change(maxBitsSelect, { target: { value: '4' } })

    await act(async () => {
      resolveState?.(new Response(JSON.stringify(buildStateResponse(8)), { status: 200 }))
      await stateResponse
    })

    await waitFor(() => {
      assert.equal(maxBitsSelect.value, '4')
    })
  } finally {
    if (act) {
      await act(async () => {
...
void test('BinaryBreachManager keeps local edits when roster websocket messages arrive', { concurrency: false }, async () => {
  const sockets: TestSocket[] = []
  const restoreDom = installDomEnvironment('https://bits.example/manage/binary-breach/session-one', sockets)
  const previousFetch = globalThis.fetch

  globalThis.fetch = (async (input) => {
    const url = String(input)
    if (url.endsWith('/state')) {
      return new Response(JSON.stringify(buildStateResponse(8)), { status: 200 })
    }
    throw new Error(`Unexpected fetch: ${url}`)
  }) as typeof fetch
  ...
  try {
    ...
    await act(async () => {
      sockets[0]?.onmessage?.({
        data: JSON.stringify({
          type: 'binary-breach:roster',
          payload: buildStateResponse(8),
        }),
      } as MessageEvent)
      await Promise.resolve()
    })
