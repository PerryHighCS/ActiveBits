...
import type { HostedFileAssignment } from '../../wwwSimTypes.js'

const okHeader = 'HTTP/1.1 200 OK\nContent-Type: text/plain'

interface HostedFileChipProps {
  fileName: string
  fragment: string
  header?: string
}

function HostedFileChip({ fileName, fragment, header = okHeader }: HostedFileChipProps) {
  ...
  function handleDragStart(event: DragEvent<HTMLDivElement>): void {
    event.dataTransfer.setData('text/plain', fullContent)
  }
...
interface StudentHostPaletteProps {
  fragments?: HostedFileAssignment[]
  hostname: string
}

export default function StudentHostPalette({ fragments = [], hostname }: StudentHostPaletteProps) {
  const errorFiles: HostedFileAssignment[] = [
    {
