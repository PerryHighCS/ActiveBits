...
void test('buildBrythonRunnerHtml runs user code in a Brython worker', () => {
  ...
  assert.match(html, /self\.mobcodeResolveInputFuture = \(future, value\) => \{/)
  assert.match(html, /const resolve = future\?\._methods\?\.resolve;/)
  assert.match(html, /if \(typeof resolve !== 'function'\) throw new Error\('Brython Future resolver unavailable'\);/)
  assert.match(html, /resolve\(value\);/)
  assert.match(html, /future\._done = true;/)
  assert.match(html, /def resolve_input_future\(input_future, value\):/)
  assert.match(html, /worker_self\.mobcodeResolveInputFuture\(input_future, value\)/)
  assert.match(html, /input_future\.set_result\(value\)/)
  assert.match(html, /input_future\.set_exception\(fallback_error\)/)
  assert.match(html, /Interactive input could not be delivered/)
  assert.match(html, /input_futures\.get\(request_id, None\)/)
  assert.match(html, /if resolve_input_future\(input_future, str\(message\.get\('value', ''\)\)\):/)
  assert.match(html, /input_futures\.pop\(request_id, None\)/)
  assert.match(html, /resolve_input_future\(input_future, str\(message\.get\('value', ''\)\)\)/)
...
void test('buildBrythonRunnerHtml wires terminal input through async worker messages', () => {
  ...
  assert.match(html, /input_future\.set_result\(value\)/)
  assert.match(html, /input_future\.set_exception\(fallback_error\)/)
  assert.match(html, /resolve_input_future\(input_future,/)
...
void test('buildBrythonRunnerHtml prints a user-file error header before the raw traceback', () => {
  ...
  assert.match(html, /return mobcode_format_exception\(error\)/)
