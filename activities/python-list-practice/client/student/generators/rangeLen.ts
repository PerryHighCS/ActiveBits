...
import { buildListFinalChoices, buildChoicePool } from './choices.js'
import { randomItem } from './utils.js'
import { WORD_LISTS } from './pools.js'

export default function rangeLenOp(
  baseList: unknown[],
  listName: string,
  useWords: boolean,
): {
  ...
  choices: unknown[]
  variant: string
  rangeLenInfo: { start: number; stop: number; step: number }
  doubleLoopInfo: { type: string; loops: Array<{ type: string; start: number; stop: number; step: number; prints: string }> } | null
} {
  ...
  const buildRangeCall = (startValue: number, stepValue: number, showZeroStart: boolean, includeStepWhenOne: boolean): string => {
  ...
  if (useValues) {
    ...
    const rangesMatch = primaryIndices.length === secondaryIndices.length && primaryIndices.every((value, index) => value === secondaryIndices[index])
    ...
    const combinedIndices = includeSecondaryValues || includeSecondaryIndices ? [...primaryIndices, ...secondaryIndices] : primaryIndices
    ...
    const expectedSequence = includeSecondaryIndices
      ? [...primaryIndices.map((idx) => String(workingList[idx])), ...secondaryIndices.map((idx) => String(idx))].join(',')
      : combinedIndices.map((idx) => String(workingList[idx])).join(',')
    ...
    const loopsInfo: Array<{ type: string; start: number; stop: number; step: number; prints: string }> = [
  ...
  const numberChoices = buildListFinalChoices(numberChoicesBase, false, null, null, Math.min(12, selectionList.length + workingList.length))
  const rangesMatch = primaryIndices.length === secondaryIndices.length && primaryIndices.every((value, index) => value === secondaryIndices[index])
  ...
  const numbersExpected = includeValuesInNumbers
    ? [...primaryIndices.map((idx) => String(idx)), ...secondaryIndices.map((idx) => String(workingList[idx]))].join(',')
    : [...primaryIndices.map((idx) => String(workingList[idx])), ...secondaryIndices.map((idx) => String(idx))].join(',')
  return {
    ...
    doubleLoopInfo: {
      type: 'range-len',
      loops: [
        { type: 'range-len', start: primaryRangeInfo.start, stop: primaryRangeInfo.stop, step: primaryRangeInfo.step, prints: includeValuesInNumbers ? ('index' as const) : ('value' as const) },
        { type: 'range-len', start: secondaryRangeInfo.start, stop: secondaryRangeInfo.stop, step: secondaryRangeInfo.step, prints: includeValuesInNumbers ? ('value' as const) : ('index' as const) },
      ],
    },
  }
}
